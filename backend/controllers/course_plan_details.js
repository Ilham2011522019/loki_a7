...
controllers.hapusPertemuan = async(req, res) => {
    try {
        ...
        res.status(200).redirect("/Pertemuan/"+id+"/"+name) 
    }
    catch (err) {
        console.log(err);
    }
...
controllers.Pertemuan = async (req, res) => {
...
controllers.hlmneditPertemuan = async (req, res) => {
    const id = req.params.id
    const name = req.params.name
...
controllers.hlmntambahPertemuan = async (req, res) => {
    const id = req.params.id
    const name = req.params.name
    const accessToken = req.cookies.accessToken 
    if (!accessToken)
